...
U1 = 4.41; % m/s
scale = 1;
nx = 100*scale;  % number of x nodes
ny = 100*scale;  % number of y nodes
...
%%
if plot_vert == 1
    % plot verticl velocity profile
    ind = find(X(1,:)<=L1);
    start = ind(end);
    stop = start + 10;
    plot(Y(:,start:stop),u(:,start:stop));
    max(max(u(:,start:stop)))
end
%%
if plot_quiv == 1
    figure
    ...
    ax1 = subplot(2,1,1);
    contourf(ax1,X,Y,PSI),colormap(ax1,'bone')
...
% figure; plot(u(:,length(u)),Y(:,length(u)));